...
const OrderItem: React.FC<OrderItemProps> = ({ order, active }) => {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 flex items-end md:items-center justify-center z-[70] p-4">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              ...
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-2xl space-y-3">
                  ...
                  {order.items.map((item, index) => (
                    <div
                      key={index}
                      className="flex justify-between items-center border-b border-gray-100 dark:border-white/5 last:border-0 pb-2 last:pb-0"
                    >
                      <div>
                        <p className="text-sm font-bold">{item.name}</p>
                        <p className="text-xs text-gray-500">
                          {item.quantity}x {item.price} AOA
                        </p>
                      </div>
                    </div>
                  ))}
