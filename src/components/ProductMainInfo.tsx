...
import DispatchProductPanel from "./DispatchProductPanel";

interface Props {
  product: SearchedProductDTO;
  dispatchStatus?: DispatchStatusResponseDto | null;
}

const ProductMainInfo: React.FC<Props> = ({ product, dispatchStatus }) => {
  const showDispatchInfo = product.isDispatch && dispatchStatus;
  ...
  return (
    <div className="bg-card p-5 rounded-xl shadow-xl border border-border">
      ...
      {showDispatchInfo ? (
        <DispatchProductPanel dispatchStatus={dispatchStatus} />
      ) : (
      ...
      <div className="mt-4 flex items-center justify-between border-t pt-4">
        <div
          className={`flex items-center gap-1.5 ${
            productConditionColor[product.condition]
          }`}
        >
