...
const TralloInput: React.FC<TralloInputProps> = ({
  ...
  const borderStyles: Record<ValidationState, string> = {
    default: "border-border focus-within:border-primary focus-within:ring-4 focus-within:ring-primary/10",
    error: "border-red-500 focus-within:border-red-500 focus-within:ring-4 focus-within:ring-red-500/10",
    valid: "border-green-500 focus-within:border-green-500 focus-within:ring-4 focus-within:ring-green-500/10",
  };
  ...
  const renderIcon = () => {
    ...
    switch (icon) {
      case "inventory_2": return <Package className={iconClass} size={20} />;
      case "report_problem": return <AlertTriangle className={iconClass} size={20} />;
      case "search": return <Search className={iconClass} size={20} />;
      case "mail": return <Mail className={iconClass} size={20} />;
      case "lock": return <Lock className={iconClass} size={20} />;
      case "person": return <User className={iconClass} size={20} />;
      default: return <Info className={iconClass} size={20} />;
    }
  };
        ...
        {isSelect ? (
          <div className="relative">
            <div
              ...
              onBlur={onBlur}
            >
              <span className={inputValue ? "text-foreground" : "text-muted-foreground/60"}>
                {options.find((opt) => opt.value === inputValue)?.label || placeholder || "Selecione uma opção"}
              </span>
            </div>
